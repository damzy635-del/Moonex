...
This app's `/api/chat`, `/api/research`, and `/api/code/run` all route
through **My AI's unified API** instead of calling Google Gemini directly.
This is what "runs on the API key" means
...
`/api/research` and `/api/code/run` are single-shot (non-streaming) calls,
so they share `lib/myai-gateway.ts` rather than duplicating `/api/chat`'s
streaming logic: same credential handling, provider ranking, retry/error
classification, and rate limiting, via `completeNonStreaming()`. Research
still runs its two-stage plan-then-report flow, with `enable_search: true`
on the report stage; code execution runs a single simulate/lint completion
on `Moonex Code 1.5`. Both delegate through `server.ts` for local dev, the
same way `/api/chat` and `/api/models` do, so dev and prod can't drift.

## Not migrated (by design)

- `/api/tts` is explicitly out of scope and still calls Gemini directly —
  audio synthesis isn't a capability My AI has, and building it wasn't
