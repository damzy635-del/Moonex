    ...
    >
      <div className="mx-auto flex max-w-3xl gap-3 sm:gap-4">
        ...
        <div className="flex-1 overflow-hidden space-y-3">
          ...
          {/* Thinking Accordion (Chain of Thought with Copy/Export) */}
          {!isUser && (message.thoughtContent || message.thinkingTimeMs) && (
            <div className="rounded-xl border border-purple-900/50 bg-purple-950/20 text-xs overflow-hidden">
              <div
                ...
                className="flex w-full items-center justify-between px-3 py-2 text-purple-300 font-medium hover:bg-purple-900/30 transition-colors cursor-pointer select-none"
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="p-1 text-purple-300 hover:text-white hover:bg-purple-900/50 rounded transition-colors"
                  ...
                  <button
                    type="button"
                    onClick={handleDownloadThinking}
                    className="p-1 text-purple-300 hover:text-white hover:bg-purple-900/50 rounded transition-colors"
                    title="Download reasoning note"
                  >
          ...
          {isEditing ? (
            <form onSubmit={handleSaveEdit} className="space-y-2 pt-1">
              <textarea
                ...
                rows={3}
                className="w-full rounded-xl border border-indigo-500/70 bg-gray-900 px-3.5 py-2.5 text-sm text-gray-100 placeholder:text-gray-500 focus:border-indigo-400 focus:outline-hidden"
              />
              <div className="flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setIsEditing(false)}
                  className="rounded-lg px-3 py-1.5 text-xs text-gray-400 hover:bg-gray-800 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="rounded-lg bg-indigo-600 px-3.5 py-1.5 text-xs font-semibold text-white hover:bg-indigo-500 transition-colors shadow-xs"
                >
                  Save & Resubmit
                </button>
              </div>
          ...
          {!isStreaming && !isEditing && (
            <div className="flex flex-wrap items-center gap-1 pt-1.5 text-gray-400 text-xs">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-md p-1.5 hover:bg-gray-800 hover:text-gray-200 transition-colors"
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-md px-2 py-1 transition-colors ${
                      isPlayingAudio
                        ? 'text-indigo-300 bg-indigo-950/80 border border-indigo-800 font-medium'
                    ...
                    {isPlayingAudio ? (
                      <>
                        ...
                        <span className="flex items-center gap-0.5 ml-1">
                          <span className="h-2 w-0.5 bg-indigo-400 animate-bounce" style={{ animationDelay: '0ms' }} />
                          <span className="h-3 w-0.5 bg-indigo-400 animate-bounce" style={{ animationDelay: '150ms' }} />
                          <span className="h-2 w-0.5 bg-indigo-400 animate-bounce" style={{ animationDelay: '300ms' }} />
                        </span>
                      </>
                    ) : (
                      <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-mono text-gray-400 hover:text-gray-100 hover:bg-gray-800 border border-gray-800 transition-colors"
                    title="Click to cycle audio playback speed (1x, 1.25x, 1.5x, 2x)"
                  >
              ...
              {onTogglePinMessage && (
                <button
                  ...
                  className={`flex items-center gap-1 rounded-md p-1.5 hover:bg-gray-800 transition-colors ${
                    message.isPinned ? 'text-amber-400' : 'hover:text-gray-200'
                  }`}
              ...
              {onBranchConversation && (
                <button
                  type="button"
                  onClick={() => onBranchConversation(message.id)}
                  className="flex items-center gap-1 rounded-md p-1.5 hover:bg-gray-800 hover:text-gray-200 transition-colors"
                  title="Fork conversation from this message"
                >
                  <GitBranch className="h-3.5 w-3.5" />
                  <span className="text-[11px] hidden sm:inline">Branch</span>
                </button>
              )}
                ...
                <>
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-md hover:bg-gray-800 transition-colors ${
                      feedback === 'up' ? 'text-emerald-400' : 'hover:text-gray-200'
                    }`}
                    title="Good response"
                  >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-md hover:bg-gray-800 transition-colors ${
                      feedback === 'down' ? 'text-rose-400' : 'hover:text-gray-200'
                    }`}
                    title="Bad response"
                  >
                  ...
                  {onRegenerate && (
                    <button
                      type="button"
                      onClick={onRegenerate}
                      className="flex items-center gap-1 rounded-md p-1.5 hover:bg-gray-800 hover:text-gray-200 transition-colors ml-auto"
              ...
              <div className="ml-auto opacity-0 group-hover:opacity-100 text-[10px] text-gray-500 font-mono transition-opacity">
...
// Code Block with Canvas, Run, and Collapsible Long Content
    ...
    <div className="my-3 overflow-hidden rounded-xl border border-gray-800 bg-[#121212] text-gray-100 shadow-md">
      ...
      <div className="flex items-center justify-between border-b border-gray-800 bg-[#1a1a1a] px-3.5 py-1.5 text-xs text-gray-400">
        <div className="flex items-center gap-2 font-mono">
          <span className="font-medium lowercase text-gray-300">
            {language}
          </span>
      ...
      {/* Code Body with collapsible clamp */}
      <div className="relative">
        <pre
          className={`overflow-x-auto p-4 font-mono text-[13px] leading-relaxed text-gray-200 ${
            isLong && !isExpanded ? 'max-h-[380px] overflow-hidden' : ''
          }`}
        >
          <code>{code}</code>
        </pre>

        {/* Collapsed Gradient Overlay and Expand Button */}
        {isLong && !isExpanded && (
          <div className="absolute inset-x-0 bottom-0 flex h-20 items-end justify-center bg-gradient-to-t from-[#121212] via-[#121212]/80 to-transparent pb-3">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-gray-800/90 border border-gray-700/80 px-3 py-1.5 text-xs font-medium text-gray-200 hover:bg-gray-700 hover:text-white shadow-lg backdrop-blur-xs transition-colors"
            >
