...
function providerCatalogHealth(provider) {
  const explicitHealthy = provider?.healthy ?? provider?.health?.healthy ?? provider?.available;
  const explicitLatency = provider?.latency_ms ?? provider?.latencyMs ?? provider?.health?.latencyMs;
  const health = providerHealth(provider?.id);
  if (explicitHealthy === false) return { ...health, score: Math.min(health.score, 0.15), cooldown: true };
  if (explicitHealthy === true) return { ...health, score: Math.max(health.score, 0.85) };
  if (Number.isFinite(Number(explicitLatency))) {
    const latency = Math.max(1, Number(explicitLatency));
    const latencyScore = 1 / (1 + Math.max(0, latency - 250) / 1_500);
    return { ...health, score: health.score * 0.65 + latencyScore * 0.35, latencyMs: latency };
  }
  return health;
}
  ...
  const qualityRanked = pool
    .map((provider, index) => {
      ...
      for (let keywordIndex = 0; keywordIndex < keywords.length; keywordIndex += 1) {
        ...
        const preference = keywords.length - keywordIndex;
        if (id === keyword) score += 100 + preference;
        else if (id.includes(keyword)) score += 50 + preference;
        else if (haystack.includes(keyword)) score += 20 + preference;
      }
      const contextLength = Number(provider.context_length ?? provider.contextLength ?? 0);
      if (Number.isFinite(contextLength) && contextLength > 0) score += Math.min(10, contextLength / 32_000);
      return { provider, score, index };
    })
    .sort((a, b) => b.score - a.score || a.index - b.index)
    .map(({ provider }) => provider);

  const catalogHealth = qualityRanked.map((provider) => {
    const health = providerCatalogHealth(provider);
    return { provider, health };
  });
  const healthAware = rankProvidersWithHealth(profile, qualityRanked);
  const byId = new Map(catalogHealth.map(({ provider, health }) => [String(provider.id), health]));
  return healthAware.sort((a, b) => {
    const ah = byId.get(String(a.id));
    const bh = byId.get(String(b.id));
    if (ah?.cooldown !== bh?.cooldown) return ah?.cooldown ? 1 : -1;
