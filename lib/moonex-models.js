...
export function decideMoonexRoute(context={}) {
  ...
  const arithmetic=/\b(what(?:'s| is)?\s+(?:\d[\d,]*(?:\.\d+)?\s*(?:[+\-*/×÷]|plus|minus|times|divided by)\s*\d[\d,]*(?:\.\d+)?|\d[\d,]*(?:\.\d+)?\s*(?:[+\-*/×÷]|plus|minus|times|divided by)\s*\d[\d,]*(?:\.\d+)?)|\bcalculate\s+\d|\b\d[\d,]*(?:\.\d+)?\s*(?:[+\-*/×÷]|plus|minus|times|divided by)\s*\d[\d,]*(?:\.\d+)?\b/i.test(text);
  const scores=new Map(MOONEX_MODELS.map(m=>[m.id,0]));
  scores.set('moonex-code-1.5',code*2.1); scores.set('moonex-reasoning-1.5',reasoning*2.2); scores.set('moonex-ultra-1.5',reasoning+complexity*8); scores.set('moonex-pro-max-1.5',reasoning*.8+complexity*6); scores.set('moonex-pro-1.5',complexity*7+creative); scores.set('moonex-fast-1.5',simple+(complexity<.25?3:0)); scores.set('moonex-lite-1.5',simple*1.2+(words<=24?3:0)+(arithmetic?8:0));
  ...
  const reason=low&&!simpleWinner?'ambiguous prompt; selected balanced profile to avoid over-routing':profile.id==='moonex-code-1.5'?'software or technical intent detected':['moonex-reasoning-1.5','moonex-ultra-1.5','moonex-pro-max-1.5'].includes(profile.id)?'deep reasoning or complex analysis detected':profile.id==='moonex-pro-1.5'?'moderate complexity or creative/general work detected':profile.id==='moonex-lite-1.5'?(arithmetic?'straightforward arithmetic task detected':'short or straightforward task detected'):'fast general task detected';
