...
const arithmeticIntent = t => /(?:\bwhat(?:'s| is)?\s+\d[\d,]*(?:\.\d+)?\s*(?:[+\-*/×÷]|plus|minus|times|divided by)\s*\d[\d,]*(?:\.\d+)?\b)|(?:\bcalculate\s+\d)|(?:\b\d[\d,]*(?:\.\d+)?\s*(?:[+\-*/×÷]|plus|minus|times|divided by)\s*\d[\d,]*(?:\.\d+)?\b)/i.test(t);
