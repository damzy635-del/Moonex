...
export function observeProviderCatalog(providers) {
  if (!Array.isArray(providers)) return;
  for (const provider of providers) {
    const id = provider?.id;
    if (!id) continue;
    const healthy = provider?.healthy ?? provider?.health?.healthy ?? provider?.available;
    const latencyMs = provider?.latency_ms ?? provider?.latencyMs ?? provider?.health?.latencyMs;
    if (healthy === true) recordProviderOutcome(id, { success: true, latencyMs });
    else if (healthy === false) recordProviderOutcome(id, { success: false, latencyMs });
    else if (Number.isFinite(Number(latencyMs))) recordProviderOutcome(id, { success: true, latencyMs });
  }
}
