...
type Msg = { role?: string; content?: unknown; files?: Array<{ data?: string; mimeType?: string }> };
...
function rateLimit(req: any) {
  ...
  return { allowed: current.count <= RATE_LIMIT, remaining, retryAfter: Math.max(1, Math.ceil((RATE_WINDOW_MS - (now - current.started)) / 1000)) };
}
...
async function liveProviders(base: string, token: string): Promise<ProviderModel[]> {
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (!payload) continue;
        if (payload === '[DONE]') { send(res, { type: 'done', modelUsed: profile.id, providerModel: provider.id }); continue; }
        try {
          const chunk: any = JSON.parse(payload);
          if (chunk?.error) { send(res, { type: 'error', error: chunk.error?.message || String(chunk.error), moonexModel: profile.id }); continue; }
    ...
    send(res, { type: 'done', modelUsed: profile.id, providerModel: provider.id });
  } catch (error) {
    console.error('Moonex /api/chat failed:', error);
    send(res, { type: 'error', error: error instanceof Error ? error.message : String(error), moonexModel: profile.id });
  } finally {
