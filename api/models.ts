...
export default async function handler(req: any, res: any) {
  ...
  try {
    const upstream = await fetch(`${base}/models`, { headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' } });
    ...
    const providers: ProviderModel[] = list.map((item: any) => typeof item === 'string' ? { id: item } : item?.id ? { ...item, id: String(item.id) } : null).filter(Boolean);
    const models = MOONEX_MODELS.map((profile) => {
      const provider = resolveProviderModel(profile, providers);
      return { id: profile.id, name: profile.name, description: profile.description, temperature: profile.temperature, maxTokens: profile.maxTokens, available: !!provider };
